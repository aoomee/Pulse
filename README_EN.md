...
[中文](README.md) · [Download prerelease](https://github.com/aoomee/Pulse/releases/tag/v1.4.0-vnstat.6) · [Container image](https://github.com/aoomee/Pulse/pkgs/container/pulse) · [Build checks](https://github.com/aoomee/Pulse/actions/workflows/publish-fork.yml) · [MIT](LICENSE)

## Current version

`v1.4.0-vnstat.6` is a published **prerelease for deployment testing**. The public image supports Linux amd64 / arm64 and does not require a GHCR login.

```text
ghcr.io/aoomee/pulse:1.4.0-vnstat.6
```
...
mkdir pulse
cd pulse
curl -fL https://github.com/aoomee/Pulse/releases/download/v1.4.0-vnstat.6/docker-compose.yaml -o docker-compose.yaml
docker compose pull
docker compose up -d
docker compose ps
```
...
The installer defaults to this fork’s `v1.4.0-vnstat.6`, with the binary at `/opt/pulse/pulse-server`, data at `/opt/pulse/data`, and port 8008. Do not run it alongside a Docker deployment on the same port. For an existing installation, back up first and stop `pulse-server` before running the installer.
...
For manual installation, use the matching `pulse-server-standalone-linux-amd64` or `pulse-server-standalone-linux-arm64` asset from the [Release](https://github.com/aoomee/Pulse/releases/tag/v1.4.0-vnstat.6), which also contains clients, installers and `SHA256SUMS`.
...
3. **Configure docker-compose.yaml to enable IPv6**
   ...
   services:
     pulse:
       image: ghcr.io/aoomee/pulse:1.4.0-vnstat.6
...
```bash
curl -fsSL https://github.com/aoomee/Pulse/releases/download/v1.4.0-vnstat.6/install.sh | bash -s -- --id 'YOUR_ID' --server 'SERVER_URL' --secret 'YOUR_SECRET'
```
...
```bash
curl -fsSL https://github.com/aoomee/Pulse/releases/download/v1.4.0-vnstat.6/install.sh | bash -s -- --id 'YOUR_ID' --server 'SERVER_URL' --secret 'YOUR_SECRET' --vnstat --traffic-reset-day 8
```
...
```bash
curl -fsSL https://github.com/aoomee/Pulse/releases/download/v1.4.0-vnstat.6/install.sh | sudo bash -s -- --id 'YOUR_ID' --server 'SERVER_URL' --secret 'YOUR_SECRET'
```
...
irm https://github.com/aoomee/Pulse/releases/download/v1.4.0-vnstat.6/install.ps1 | iex
```
...
# curl -sSL https://github.com/aoomee/Pulse/releases/download/v1.4.0-vnstat.6/docker-compose.yaml -o docker-compose.yaml && \
# docker compose up -d && \
# curl -fsSL https://raw.githubusercontent.com/aoomee/Pulse/v1.4.0-vnstat.6/scripts/migrate.sh -o migrate.sh && chmod +x migrate.sh
