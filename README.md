...
[English](README_EN.md) · [下载测试版](https://github.com/aoomee/Pulse/releases/tag/v1.4.0-vnstat.12) · [容器镜像](https://github.com/aoomee/Pulse/pkgs/container/pulse) · [构建检查](https://github.com/aoomee/Pulse/actions/workflows/publish-fork.yml) · [MIT](LICENSE)

## 当前版本

新增 Telegram 通知，支持自定义离线等待时间与消息模板，见[通知配置说明](docs/telegram.md)。后台点击铃铛进入设置；默认关闭，升级后按需启用。

`v1.4.0-vnstat.12` 是本轮 **Pre-release 测试版**：新增流量校准就绪红绿灯，修复编辑服务按钮和报错的中文显示，校准输入框默认留空；保留统一的玻璃与普通样式。[发布任务](https://github.com/aoomee/Pulse/actions/workflows/publish-fork.yml)成功后，镜像和下载文件即可使用。镜像支持 Linux amd64 / arm64，公开拉取，无需登录 GHCR。
...
- Telegram 设置的输入框、下拉框和文字按钮统一为 44px 高度、14px 圆角；预览区三个控件等宽对齐，保留离线 / 恢复 / 测试选项。

- 新增 Telegram 离线 / 恢复通知：等待时间可设为 15 秒至 24 小时，支持单台静音。
- 新增 JavaScript 消息模板编辑、无发送预览和测试发送，兼容 Komari 风格的 sendMessage / sendEvent 入口。
- 通知限时执行、有限重试和状态持久化；配置仅管理员可见，令牌保存后不回传。

- 展开详情整体居中，按 CPU、硬件资源和网络流量分组，手机自动重排。
- 首页进度条底轨与填充统一为 9px 圆角。
- 后台主机信息两行对齐，编辑和复制按钮使用圆形底框；编辑按钮居中于两行之间，长 IP 省略显示但完整复制。
- 首页标题简化为“服务器”，移除说明文字；加载使用无文字细线圆环，支持减少动态效果。
- “列”菜单文字居中，移除排序箭头，简化圆角与间距；Esc 关闭后焦点返回按钮。
- 刷新时统一为轻柔淡入，等待首批数据与主题就绪后展示；取消导航、图标和列表的重复入场，慢连接仅显示简洁提示。
...
安装器默认使用本 fork 的 `v1.4.0-vnstat.12`，程序位于 `/opt/pulse/pulse-server`，数据位于 `/opt/pulse/data`，端口 8008。不要与同端口的 Docker 部署同时运行。已有安装升级前先备份，并停止 `pulse-server` 服务后再运行安装器。
...
手动下载请到 [Release](https://github.com/aoomee/Pulse/releases/tag/v1.4.0-vnstat.12) 选择 `pulse-server-standalone-linux-amd64` 或 `pulse-server-standalone-linux-arm64`。Release 同时提供客户端、安装脚本和 `SHA256SUMS`。
